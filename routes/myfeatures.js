...
/**
 * GET a list of features which belongs to the owner. User ID is fetched from JWT token
 */
router.get('/', function (req, res, next) {
  ...
  User.findOne({_id: id}, 'username')
    ...
    .then(function (data) {
      ...
      return Feature.find(requestOptions).deepPopulate('featureGroup attributeInstances featureGroup.attributeInstances attributeInstances.attribute featureGroup.attributeInstances.attribute')
    })
    .then(function (docs) {
...
/**
 *  Adds a Feature object to Feature collection.
 *  Expects the payload of the attribute instances to be in the form [{attribute._id,value},{attribute._id,value}]
 */
router.post('/', utils.checkPermissions("features", "create"), function (req, res, next) {
  var feature;
  var data = {};
